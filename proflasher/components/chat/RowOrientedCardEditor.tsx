	...
	return (
		<div className={`space-y-4 ${className}`}>
						...
						{(() => {
							...
							const fieldOrder = [
								...
								...Object.keys(card.fields)
									.filter((f) => !["Key", "Mnemonic", "Related"].includes(f))
									.sort(),
								"Mnemonic",
								"Related",
							];
							...
							const orderedFields = fieldOrder.filter((field) =>
								Object.hasOwn(card.fields, field),
							);
							...
							return orderedFields.map((field) => (
								...
								>
									<label className="min-w-20 flex-shrink-0 font-medium text-gray-600 text-sm">
										{field}:
									</label>
									<textarea
										ref={(el) => {
			...
			{tableDefinitions.map((tableDef) => {
				...
				const rowNames = sortRowNames(
					Object.keys(tableData),
					tableDef.rowDescriptions,
				);
											...
											<tr key={rowName} className="border-t">
												<td className="border-r px-2 text-center">
													<div className="flex items-center justify-center gap-1">
														<button
															onClick={() =>
																moveRow(tableDef.name, rowName, "up")
															}
															disabled={index === 0}
														...
														<button
															onClick={() =>
																moveRow(tableDef.name, rowName, "down")
															}
														...
														<button
															onClick={() =>
																handleRemoveRow(tableDef.name, rowName)
															}
