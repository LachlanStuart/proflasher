...
export function AnkiSearchMessage({ message }: AnkiSearchMessageProps) {
    ...
    const resultsToDisplay = showAllResults ? message.results : message.results.slice(0, MAX_PREVIEW_RESULTS);
        ...
        <div className="mb-4 flex">
            <div className="max-w-[80%] rounded-lg border-blue-400 border-l-4 bg-gray-100 px-4 py-2 text-gray-600">
                <div className="font-mono text-sm">
                    ...
                    <div className="mb-2">
                        Found: <span className="font-bold">{message.results.length}</span> results
                    </div>

                    {message.error && <div className="mt-2 text-red-500">Error: {message.error}</div>}
                    ...
                    {resultsToDisplay.length > 0 && (
                        <div className="mt-2 border-gray-300 border-t pt-2">
                            {resultsToDisplay.map((result: any, index: number) => (
                                ...
                                >
                                    <div className="text-gray-500 text-xs">Card ID: {result.id}</div>
                                    {result.modelName && (
                                        <div className="text-gray-500 text-xs">Model: {result.modelName}</div>
                                    )}
                                    <div className="mt-1">
