...
export function generateCardFormatExample(template: ProcessedNoteTemplate): string {
    ...
    example += "This example uses JSON for demonstration, but you should use the tool call. Never send JSON to the user directly. They will explode.\n"
...
export function generateTablePrompt(template: ProcessedNoteTemplate): string {
    ...
    prompt += "- Think before populating each card if you need to. You may add an extra 'Thinking' field to cards that will be ignored.\n";
