 ...
 */
export function generateTableStructurePrompt(
	...
	if (tableDefinitions.length === 1) {
		...
		for (const column of table.columns) {
			const description =
				table.columnDescriptions?.[column] || `${column} language content`;
		...
		if (Object.keys(table.rowDescriptions).length > 0) {
			...
			for (const [rowName, description] of Object.entries(
				table.rowDescriptions,
			)) {
 ...
 */
export function generateCardFormatExample(
...
export function generateTablePrompt(template: ProcessedNoteTemplate): string {
	...
	prompt +=
		"- Don't leave empty rows - only include rows with actual content\n";
	prompt +=
		"- If creating cards, call `proposeCards` immediately. Don't speak before calling it.";
