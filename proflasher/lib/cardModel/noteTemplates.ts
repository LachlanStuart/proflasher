...
export const cards = ({
	...
	fieldLangs,
	cards,
}: NoteTemplate): Record<string, CardModel> => {
	...
	for (const [cardName, { rows }] of Object.entries(cards)) {
		...
		for (const { flag, field, attrs } of rows) {
			r.push(
				`<tr><td><div class="${flag}"></div></td><td ${attrs} data-field-display="${field}"></td></tr>`,
			);
		}
		r.push("</table>");
		r.push(
		...
		for (const column of group) {
			if (column === "EN") {
				...
				columnDescriptions[column] =
					"French text (words, phrases, or sentences)";
			} else if (column.includes("FR")) {
				columnDescriptions[column] =
					"French conjugations, declensions, or forms";
			} else if (column === "DE") {
				columnDescriptions[column] =
					"German text (words, phrases, or sentences)";
			} else if (column === "ZH") {
	...
	for (const entry of entries) {
		...
		try {
			...
			const tableDefinitions =
				template.tableDefinitions ||
				generateTableDefinitionsFromFieldGroups(
...
export async function validateNote(
	...
	if (!template) {
		...
		if (missingNonTableFields.length > 0) {
			errors.push(
				`Missing required non-table fields: ${missingNonTableFields.join(", ")}.`,
			);
		}
