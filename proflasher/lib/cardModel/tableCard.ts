...
export type RowOrientedCard = {
    ...
    fields: Record<string, string>; // Non-table fields like Key, UsageNotes, Related
};
...
export function rowToColumnOriented(rowCard: RowOrientedCard, tableDefinitions: TableDefinition[]): ColumnOrientedCard {
    ...
    for (const [tableName, rowOrder] of Object.entries(tableRowOrders)) {
        const orderFieldName =
            tableName === "main" ? "RowOrder" : `${tableName.charAt(0).toUpperCase() + tableName.slice(1)}RowOrder`;
 ...
 */
export function columnToRowOriented(
    columnCard: ColumnOrientedCard,
    ...
    for (const tableDef of tableDefinitions) {
        ...
        const rowNames = rowOrderValue
            .split(";")
            .filter((name) => name.trim())
            .map((name) => name.replace(/\.,/g, ";")); // Restore semicolons in row names
 ...
 */
export function removeRowFromTable(rowCard: RowOrientedCard, tableName: string, rowName: string): RowOrientedCard {
    const updated = structuredClone(rowCard);
