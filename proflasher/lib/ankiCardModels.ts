...
import { type CardModel, type Templates, loadTemplates } from "./cardModel/noteTemplates";
import { env } from "./env";
...
const getAnkiNotes = async (): Promise<Notes> =>
    Object.fromEntries(await Promise.all((await Anki.modelNames()).map(getAnkiNoteModel)));
...
export const updateCardModels = async (
    ...
    const jsCode = await fsPromises.readFile(path.join(__dirname, "../lib/cardModel/card.js"), "utf-8");
    const cssCode = await fsPromises.readFile(path.join(__dirname, "../lib/cardModel/card.css"), "utf-8");
    ...
    for (const [lang, template] of templatesToUpdate) {
        ...
        if (ankiNote) {
            ...
            for (const [cardName, localSides] of Object.entries(template.cards)) {
                ...
                const Front = fixString(`${localSides.Front}\n<script>${jsCode}\nrunFront()</script>`);
                const Back = fixString(`${localSides.Back}\n<script>${jsCode}\nrunBack()</script>`);
                updates[cardName] = { Front, Back };
                hasChanges ||= Front !== fixString(ankiSides.Front) || Back !== fixString(ankiSides.Back);
            }
            if (hasChanges) {
...
export const createMissingModels = async (
    ...
    for (const [lang, template] of languageTemplates) {
        ...
        try {
            ...
            try {
                // Load base CSS
                const baseCss = await fsPromises.readFile(path.join(__dirname, "../lib/cardModel/card.css"), "utf-8");
...
export const addMissingDecks = async (
    ...
    const extraDecks = existingDeckNames.filter((deck) => languagePattern.test(deck) && !requiredDecks.has(deck));
