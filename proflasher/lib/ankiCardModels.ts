...
export const addMissingFields = async (language?: string): Promise<string[]> => {
    const logs: string[] = [];
    const log = (message: string) => {
        console.log(message);
        logs.push(message);
    ...
    try {
        ...
        for (const [lang, template] of languageTemplates) {
            ...
            try {
                ...
                log(`Required fields for ${modelName}: ${Array.from(requiredFields).join(', ')}`);

                // Find missing fields
                const missingFields = Array.from(requiredFields).filter(field => !existingFields.includes(field));

                // Check for extra fields (alert user but don't remove)
                const extraFields = existingFields.filter(field => !requiredFields.has(field));
                if (extraFields.length > 0) {
                    log(`ℹ️  Extra fields in ${modelName}: ${extraFields.join(', ')}`);
                    log(`   These are not defined in your template - please review manually`);
                }

                if (missingFields.length === 0) {
                    log(`✅ All required fields exist in ${modelName}`);
                } else {
                    log(`➕ Adding missing fields to ${modelName}: ${missingFields.join(', ')}`);
                    ...
                    for (const fieldName of missingFields) {
                        try {
                            ...
                            log(`   ❌ Failed to add field ${fieldName}: ${error instanceof Error ? error.message : String(error)}`);
                        }
                    }
                }

            } catch (error) {
                if (error instanceof Error && error.message.includes('model was not found')) {
                    log(`❌ Model ${modelName} not found in Anki`);
                } else {
                    log(`❌ Error checking fields for ${modelName}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }

    } catch (error) {
        log(`❌ Error loading templates: ${error instanceof Error ? error.message : String(error)}`);
    }

    return logs;
};

export const addMissingCards = async (language?: string): Promise<string[]> => {
    const logs: string[] = [];
    const log = (message: string) => {
        console.log(message);
        logs.push(message);
    ...
    try {
        ...
        for (const [lang, template] of languageTemplates) {
            ...
            try {
                ...
                log(`Existing card types in ${modelName}: ${existingCardNames.join(', ')}`);

                // Get required card types
                const requiredCardNames = Object.keys(template.cards);
                log(`Required card types for ${modelName}: ${requiredCardNames.join(', ')}`);

                // Find missing card types
                const missingCards = requiredCardNames.filter(cardName => !existingCardNames.includes(cardName));

                // Check for extra card types (alert user but don't remove)
                const extraCards = existingCardNames.filter(cardName => !requiredCardNames.includes(cardName));
                ...
                if (missingCards.length === 0) {
                    ...
                    log(`➕ Adding missing card types to ${modelName}: ${missingCards.join(', ')}`);
                    ...
                    for (const cardName of missingCards) {
                        try {
                            ...
                            const cardTemplate = {
                                Name: cardName,
                                Front: fixString(`${cardModel.Front}\n<script>${jsCode}\nrunFront()</script>`),
                                Back: fixString(`${cardModel.Back}\n<script>${jsCode}\nrunBack()</script>`)
                            };

                            await Anki.modelTemplateAdd(modelName, cardTemplate);
                            log(`   ✅ Added card type: ${cardName}`);
                        } catch (error) {
                            log(`   ❌ Failed to add card type ${cardName}: ${error instanceof Error ? error.message : String(error)}`);
                        }
                    }
                }
                ...
                if (error instanceof Error && error.message.includes('model was not found')) {
                    ...
                    log(`❌ Error checking card types for ${modelName}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
...
export const createMissingModels = async (language?: string): Promise<string[]> => {
    const logs: string[] = [];
    const log = (message: string) => {
        console.log(message);
        logs.push(message);
    ...
    try {
        ...
        for (const [lang, template] of languageTemplates) {
            ...
            try {
                ...
                const cardTemplates = Object.entries(template.cards).map(([cardName, cardModel]) => ({
                    Name: cardName,
                    Front: fixString(`${cardModel.Front}\n<script>${jsCode}\nrunFront()</script>`),
                    Back: fixString(`${cardModel.Back}\n<script>${jsCode}\nrunBack()</script>`)
                }));

                log(`   Card types for ${modelName}: ${cardTemplates.map(t => t.Name).join(', ')}`);
                ...
                try {
                    ...
                    css = fixString(`${baseCss}\n\n/* Language-specific styles for ${lang} */\n${customCss}`);
                } catch (error) {
                    log(`   Warning: Could not load CSS files: ${error instanceof Error ? error.message : String(error)}`);
                }
                ...
                log(`   ❌ Error creating model ${modelName}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }

    } catch (error) {
        log(`❌ Error creating models: ${error instanceof Error ? error.message : String(error)}`);
    }

    return logs;
};

export const addMissingDecks = async (language?: string): Promise<string[]> => {
    const logs: string[] = [];
    const log = (message: string) => {
        console.log(message);
        logs.push(message);
    ...
    try {
        ...
        // Find missing decks
        const missingDecks = Array.from(requiredDecks).filter(deck => !existingDeckNames.includes(deck));

        // Check for extra language-related decks (informational only)
        const languagePattern = language ? new RegExp(`.*${language.toUpperCase()}.*`, 'i') : /Lang::|JP|FR|DE|ZH/i;
        const extraDecks = existingDeckNames.filter(deck =>
            languagePattern.test(deck) && !requiredDecks.has(deck)
        );
        ...
        if (missingDecks.length === 0) {
            ...
            for (const deckName of missingDecks) {
                try {
                    ...
                    log(`   ❌ Failed to create deck ${deckName}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }

            if (missingDecks.length > 0) {
                log('');
                log('📋 REMINDER: Please check the scheduling options for newly created decks!');
                log('   Go to Tools > Manage Note Types > [Deck] > Options to configure:');
                log('   - Daily limits, learning steps, graduation intervals, etc.');
            }
        }

    } catch (error) {
        log(`❌ Error managing decks: ${error instanceof Error ? error.message : String(error)}`);
    }
