...
export default function SettingsPage() {
    ...
    const fetchConfigFiles = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch prompt.md for ${language}: ${response.statusText}`);
            }
    ...
    const handleSaveFile = async (fileName: string, content: string) => {
        ...
        setSaveMessage(null);
        try {
            const response = await fetch(`/api/settings/${language}/${fileName}`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.error || `Failed to save ${fileName}: ${response.statusText}`);
            }
    ...
    return (
        <div className="pb-4">
            ...
            <div className="mb-6">
                <label htmlFor="global-instructions" className="block font-medium text-gray-700 text-sm">
            ...
            <div className="mb-6">
                <h2 className="mb-4 font-semibold text-lg">Anki Sync Options</h2>
                <div className="space-y-4">
                    {/* Sync Options */}
                    <div className="space-y-3 rounded-md bg-gray-50 p-4">
                        <h3 className="mb-3 font-medium text-gray-700">Select sync operations:</h3>
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <div className="group relative">
                                    <span
                                        ...
                                        title="AnkiSync will force you to upload changes to the cloud, so make sure to sync all devices before selecting this"
                                    >
                                        ⚠️
                                    </span>
                                </div>
                    ...
                    {ankiLogs.length > 0 && (
                        <div className="mt-4 rounded-md bg-gray-100 p-4">
                            <h3 className="mb-2 font-medium">Sync Log:</h3>
                            <pre className="whitespace-pre-wrap text-sm">{ankiLogs.join("\n")}</pre>
                        </div>
                    )}
                </div>
            </div>
