...
import { type RowOrientedCard, rowToColumnOriented } from "~/lib/cardModel/tableCard";
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!template) {
            return NextResponse.json({ error: `Template not found for model ${modelName}` }, { status: 400 });
        }
        ...
        if (card) {
            ...
            if (!isValid) {
                return NextResponse.json({ error: `Invalid card fields: ${error}` }, { status: 400 });
            }
            ...
            return NextResponse.json({ error: "Either fields or card must be provided" }, { status: 400 });
        }
        ...
        const cardTypeToOrd = new Map(Object.keys(ankiTemplates).map((name, ord) => [name, ord]));
        ...
        if (activeCardTypes) {
            ...
            const [cardsToSuspend, cardsToUnsuspend] = cardsInfo.reduce<[number[], number[]]>(
                ([suspend, unsuspend], card) => {
                    ...
                    const cardTypeName = Array.from(cardTypeToOrd.entries()).find(([_, ord]) => ord === card.ord)?.[0];
