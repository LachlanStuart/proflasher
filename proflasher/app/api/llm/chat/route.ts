			...
			parameters: {
				...
				properties: {
					query: {
						type: "string",
						description:
							"An Anki search query (note type will be automatically added)",
					},
				},
				required: ["query"],
			},
...
async function getNotes(
	...
	try {
		...
		const noteIdsByKey = await Promise.all(
			keys.map(async (key) => [
				key,
				await Anki.findNotes(`note:${noteType} Key:${key}`),
			]),
		);
	...
	while (true) {
		...
		if (responseMessage.content) {
			...
			if (contentStr.startsWith('{"tool_call":')) {
				try {
					console.log(
						`Detected JSON in content: ${contentStr.substring(0, 100)}...`,
					);
